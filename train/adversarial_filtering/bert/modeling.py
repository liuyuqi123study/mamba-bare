...
def get_cls_output(output_layer, is_training, labels, num_labels):
    """Creates a classification model."""
    batch_size_times_num_labels, hidden_size = get_shape_list(output_layer, expected_rank=2)
    ...
    with tf.variable_scope("cls/nwayloss"):
        if is_training:
            # I.e., 0.1 dropout
        ...
        # Right now logits is shape [b*4, 1] so fix that, if training
        logits = tf.reshape(logits, [batch_size_times_num_labels//num_labels, num_labels])
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
                     ...
                     use_one_hot_embeddings, do_mask=True):
            ...
            if use_tpu:
                ...
                if do_mask:
                    logging_info = {
                        ...
                        'cls_loss': tf.metrics.mean(per_example_cls_loss)[1],
