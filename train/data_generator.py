...
def generate_dataset(dataset_config, training_config):
    ...
    return x, y
def generate_InductionDataset(num_samples=10000, seq_length=256, vocab_size=16,batch_size=8):
        inputx=[]
        targetx=[]
        for _ in range(num_samples):
            
            half_seq = [random.randint(1, vocab_size) for _ in range(seq_length // 2)]
        
            full_seq = half_seq + half_seq
            target = full_seq[1:] # Shifted sequence with padding
            inputx.append(full_seq)
            targetx.append(target)
        inputx=torch.tensor(inputx).view(batch_size,num_samples//batch_size,256)
        targetx=torch.tensor(targetx).view(batch_size,num_samples//batch_size,255)
        return inputx, targetx
